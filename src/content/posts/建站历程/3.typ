...
- Typst 原生支持 HTML 导出（虽然还在实验阶段），配合社区维护的 `astro-typst` 集成，就能让博客直接渲染 `.typ` 文件。虽然生态尚未成熟，但通过恰当的配置，也能实现极好的效果——比如这篇文章，看上去就和前面几篇的样式完全一致。你也可以看看我用 Typst 写的 #link("/posts/数学研究/随机排列问题/")[随机排列问题] 这篇文章，里面的数学公式效果也非常好。
...
Markdown 用行首的 `>` 写引用，但 Typst 并不认识这个语法：行首的 `>` 会被当成普通文本原样输出，连续多行还会被并进同一个段落，渲染出的 HTML 根本分不清哪个 `>` 才是引用标记。我也尝试过 Typst 内置的 `#blockquote` 元素，可惜当前的 HTML 导出后端并没有实现它。

既然 Typst 自己走不通，我就在编译的前后各做了一次手脚，把它和 Markdown 对齐：

1. *编译前预处理*：把源码交给编译器之前，逐行扫描，将行首（允许前导空白）的 `>` 替换成一个几乎不会在正文里出现的私有区字符（PUA）；块级数学和代码块内部会跳过，避免误伤公式里的 `>` 运算符。
2. *编译后处理*：渲染出的 HTML 中，凡是以该标记开头的段落，去掉标记并包成 `<blockquote>`，相邻的引用段落再合并为同一个引用块。
...
连续引用行并成一段、空行分隔则分段、多段并入同一个块、列表项里写引用，这些行为都和 Markdown 一致。你读到这篇文章开头那句"本篇文章的技术细节由 AI 书写"，正是引用块的渲染效果。

这个功能踩过两个坑：

- *必须在"列表解包"之前处理引用。* 前面"列表间距"一节的适配会把列表项里的段落标签解开，我一开始把引用处理放在它之后，导致列表里的引用段落已被解成裸文本、无从识别，把顺序对调之后才正常。
- *预处理后的源码不能和文件路径同时传给编译器。* Typst 编译器不允许"内存内容"与"文件路径"同时指定，否则直接报错；传入预处理后的内存内容时，把文件路径省掉即可。
...
- 引用块目前只支持单层（`>`），Markdown 的多层嵌套引用（`>>`）暂不支持。
...
- 标题大小逐级对齐（`=` 与 `#` 同级）。
- 列表项间距一致。
- 引用块左侧有竖条，连续引用行、空行分段等行为与 Markdown 一致。
- 公式能选中、能复制、颜色随主题。
