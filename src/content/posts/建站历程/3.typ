...
上篇文章的结尾，我留下了一句话："对于这类个性化的需求，下一步便是改造这个网站！"。如今，改造完成了。

于是就有了这篇文章——准确地说，有了*这篇文章的载体*。你现在读到的每一个字，都是用 Typst 书写的，由博客原生渲染出来的。标题、目录、公式、列表、代码块，无一例外。而在几天之前，这还是一桩不可能的事。

这篇文章记录了我让博客支持并适配 Typst 的完整流程。如果你也想让博客支持 Typst，跟着下面的步骤走，应该能顺利复现。

== 为什么要让博客支持 Typst

在前一篇里，我已经介绍过 Typst 这个排版系统，这里不再赘述，只补充一点实际的体验：

- 我计划为自己的博客添加数学相关内容，其中涉及大量公式的编辑和排版。Markdown 的数学公式沿袭了 LaTeX 的语法，写起来冗长、可读性低。而 Typst 的公式写起来就非常顺手，而且从代码也很容易一眼看出公式的含义。
- Typst 原生支持 HTML 导出（虽然还在实验阶段），配合社区维护的 `astro-typst` 集成，就能让博客直接渲染 `.typ` 文件。虽然生态尚未成熟，但通过恰当的配置，也能实现极好的效果。比如这篇文章，看上去就和前面几篇的样式完全一致。你还可以看看我使用 Typst 编写的 #link("/posts/数学研究/泊松分布/")[泊松分布] 这篇文章，里面的数学公式效果也非常好。
...
- `htmlMode: "text"`：把渲染结果作为纯文本 HTML 片段直接内嵌。*这个选项很关键*，它会绕过集成默认的 hast 处理，让后续的格式统一后处理变得简单可靠。这也是代码块统一（见下文）能实现的前提。
...
字段的含义和 Markdown 版本的 frontmatter 一一对应——标题、日期、描述、标签、分类、草稿开关，都在这里。写错格式集成会直接报错，大小写和括号要留意。

=== Typst 语法简介

Typst 的正文语法与 Markdown 有很多类似之处，在这里只简单介绍一点。更多细节可以查阅 #link("https://typst.app/docs/tutorial")[官方文档] 。
...
代码块同样是用三个反引号包裹，在开头的三个反引号后紧跟语言标记即可。你在这篇文章里看到的每一个代码框，包括它自身的行号、复制按钮和语言徽章，都是 Typst 文章代码块统一适配后的效果（详见下文）。
...
Markdown 文章的目录（TOC）是 Astro 的 markdown 管线自动提取的。但 Typst 文章的 render 结果里*没有标题列表*，目录得自己从源码提取。
...
Typst 的"松列表"（项与项之间留有空行）会把每个列表项的内容包进一个段落标签，导致项与项之间的间距比 Markdown 略宽。同样是在后处理中，把这些*结构性的*段落标签解开，让列表项内容直接成为文本，行距就完全一致了。

要注意留一个口子：如果某个列表项里*全都是*段落（读者真的写了多段内容），就保留段落分隔，避免把多段压成一段。

=== 公式

公式是支持 Typst 的重要理由，所以这一块务必做到位。

一开始我把公式渲染成了 SVG 图片，文本无法选中、颜色固定。后来把 `typst-ts-node-compiler` 升级到 0.8.0，才拿到*原生 MathML 输出*——公式变成真正的数学标记语言，文字可以复制，颜色随主题自动变化，和 Markdown 侧用 KaTeX 渲染的观感一致。
...
具体做法：在 `astro-typst` 渲染出 HTML 之后、返回之前，对每一个 `<pre><code data-lang="...">`，取出纯文本源码和语言，交给 expressive-code 渲染成完整的代码框结构，再替换回去，并把 expressive-code 的基础样式、主题样式和无障碍脚本注入到文章 HTML 中。于是 Typst 文章里的每个代码框，和 Markdown 文章里的在结构上完全一致——行号、复制按钮、语言徽章、主题配色，样样都有。
...
- *补丁里引不到项目源码。* 补丁文件在 `node_modules` 深处，`import` 项目 `src` 目录下的自定义插件会失败。我把两个自定义插件（语言徽章、复制按钮）的代码直接内联进了补丁。代价是：日后改 `src/plugins/` 里的插件，要记得同步补丁里的内联版本。
...
从这次的改造也可以看出，想要让博客更加适配个人需求，适当的改造是必不可少的。接下来我还会简单介绍我对博客进行的一些其它改造。