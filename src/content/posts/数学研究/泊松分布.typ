...
=== 定义
 - 引入：样方内个体数的分布
  ...
  - 数学建模：平面内布满了随机生成的点，每个点独立等可能地出现在平面内的任意位置，且平面内点的平均密度为 $lambda$ 。在一个面积为 S 的区域内，点的个数记为随机变量 X ，求 X 的分布列。
 ...
 - 唯一参数 $lambda$ 的含义
  - 均值
  - 平均密度
  - 微观本质：对于连续事件载体的微元 ds ，事件有很小概率 dp 发生，$(d p)/(d s)=lambda$
 ...
 - 理解
  ...
  - 概率为 0 的事件无数次尝试发生，积累出不为 0 的概率（类比：连续型随机变量取任何一个值的概率均为 0 ，却必然取到某个值）
  - 类似于衰变，完全不知何时发生，却仍有内在 $lambda$
 - 情境实例
  - 样方内的个体数。事件载体：二维的空间（平面）。
  - 连续版的输出机器。原先的机器输出离散的数字，每个离散的载体上，随机事件（输出 1）发生的概率为 p 。现在的机器随时间流逝随机发出一声“哔”，机器设定单位时间内发出“哔”的概率为 $lambda$ ，则一段时间内发出“哔”的次数服从泊松分布。事件载体：一维的时间。
  - 考虑函数 $f(x)$ ，其中 $x in [0,1]，forall x in [0,1]，f(x)$的值在 $[0,1]$ 中均匀随机地选取。则 $forall a in [0,1]$，$f(x)=a$ 的解的个数服从泊松分布。事件载体：一维的函数自变量。
  - 上例中改为 $f(x)=x$ ，解的个数分布亦然，这使人联想到错排问题中，匹配元素个数的分布趋于泊松分布。
...
=== 性质
 ...
 - 表达式 $(lambda^k e^(-lambda))/k!$ 对 $k$（离散）和 $lambda$（连续）的积分均为 1 ，分布与逆分布的形式相同，均值、方差、极值点等值简介，卷积封闭。这一系列的美妙性质，赋予了泊松分布独一无二的美。
...
=== 逆分布
 ...
 - $f(lambda)= (lambda^k e^(-lambda))/(integral_0^(+oo) lambda^k e^(-lambda) d lambda)=(lambda^k e^(-lambda))/k!$ 与原分布形式上相同，但离散转为连续，随机变量 $k$ 转为 $lambda$
  ...
  两边同时对 $lambda$ 取 0 到 $+oo$ 积分，左边为 0 ，右边得等式，递推至 k=1 即可
 ...
 - 卷积：$integral_0^lambda (x^a e^(-x))/a! ((lambda-x)^b e^(-(lambda-x)))/b! d x = integral_0^lambda x^a (lambda-x)^b d x dot e^(-lambda)/(a! b!) = (a! b! lambda^(a+b+1))/(a+b+1)! dot e^(-lambda)/(a! b!) = (lambda^(a+b+1) e^(-lambda))/(a+b+1)!$ 对卷积封闭！
  ...
  两边同时对 $x$ 取 0 到 t 积分，左边为 0 ，右边得等式，递推至 k=1 即可
 ...
 - 情境：考虑连续版的输出机器，机器设定的 $lambda$ 未知。观测到机器在t时间内发出n声“哔”，使用贝叶斯公式反过来推测 $lambda$ ，则 $lambda$ 的分布列即为上述的泊松分布逆分布。我们可以从该分布中得到关于 $lambda$ 的更多信息，而不仅仅是一个简单的估计值。值得注意的是，$E(lambda)=(n+1)/t$ 对比离散版的输出机器，即二项分布的逆分布，均值 +1 的奇妙修正。（联想：对一遵循特定分布的随机变量 X 进行 n 次观测，则观测样本数据的方差 $S^2$ 满足 $E(S^2)=(n-1)/n D(X)$）
...
=== 相关分布
  ...
  - 泊松分布（连续事件载体，本质上是泊松过程），事件首现分布即为指数分布 $f_t (lambda)=lambda e^(-lambda t)，E(t)=1/lambda$
   - 但是注意：泊松过程并非伯努利过程的连续版本，而是伯努利过程的累积版本的连续版本。伯努利过程是 0 阶马尔可夫/独立过程，泊松过程则是一个累积，是 1 阶马尔可夫。
   - 泊松过程之所以定义为一种累积，是因为这是连续时间上的离散事件，这种情况下只有采用累积才能良好地刻画出这一过程。不能为测度为 0 的瞬间分配一个数，就用积分函数的优良性质。混合型随机变量也有这样的困境。狄拉克 δ 函数是解决问题的一个办法。
   - 几何分布和指数分布的无记忆性：$P(T>s+t∣T>t)=P(T>s)$
  - 事件的首现分布即为间隔分布，首现期望即为间隔期望，亦为概率(密度)的倒数
  - 经 t 已发生的概率，即首现分布的累积分布函数的补数 $F(t)=e^(-lambda t)$ ，与原子衰变图相同