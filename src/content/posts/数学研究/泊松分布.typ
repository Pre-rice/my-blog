...
=== 逆分布
 ...
 - 情境：考虑连续版的输出机器，机器设定的 $lambda$ 未知。观测到机器在t时间内发出n声“哔”，使用贝叶斯公式反过来推测 $lambda$ ，则 $lambda$ 的分布列即为上述的泊松分布逆分布。我们可以从该分布中得到关于 $lambda$ 的更多信息，而不仅仅是一个简单的估计值。值得注意的是，$E(lambda)=(n+1)/t$ 对比 #link("/posts/数学研究/概率的概率/")[离散版的输出机器]，即二项分布的逆分布，均值 +1 的奇妙修正。
