...
$bold(A)=mat(delim: "[", a_11,a_12,dots,a_(1n);a_21,a_22,dots,a_(2n);dots.v,dots.v,,dots.v;a_(m 1),a_(m 2),dots,a_(m n))=mat(delim: "[","row"(bold(A),1);"row"(bold(A),2);dots.v;"row"(bold(A),m))=mat(delim: "["," col"(bold(A),1),"col"(bold(A),2),dots,"col"(bold(A),n)" ")$
...
&=mat(delim: "[","row"(bold(A),1);"row"(bold(A),2);dots.v;"row"(bold(A),m))bold(x)=mat(delim: "[","row"(bold(A),1) dot bold(x);"row"(bold(A),2) dot bold(x);dots.v;"row"(bold(A),m) dot bold(x))\ 
 
&=mat(delim: "["," col"(bold(A),1),dots,"col"(bold(A),n)" ")mat(delim: "[",x_1;dots.v;x_n)=x_1"col"(bold(A),1)+ dots +x_n"col"(bold(A),n)$
...
=== 内积空间
 ...
 - *欧几里得空间* $RR^n$：$⟨bold(x),bold(y)⟩=bold(x)^T bold(y)$，*正交矩阵* $bold(Q)^T bold(Q) = bold(I)$
  - *正交变换*保持内积，从而保持范数、角度和距离
...
=== 子空间
 ...
 - 对于任意 $m×n$ 实矩阵 $bold(A)$（线性变换 $bold(A) bold(x)=bold(b)$，$bold(x) in RR^n$，$bold(b) in RR^m$），定义四个基本子空间：
  ...
  - *行空间* $R(bold(A))=C(bold(A)^T)$：行向量张成的空间
...
=== 二次型
 - *定义*：设 $bold(A)$ 是 $n$ 阶实对称矩阵，$bold(x)$ 是 $n$ 元列向量，则 $bold(x)^T bold(A) bold(x)$ 是一个*二次型*
  ...
  - 二次型即 $n$ 个变量 $x_1, x_2, dots, x_n$ 的二次齐次多项式，二次型 $bold(x)^T bold(A) bold(x)$ 与实对称矩阵 $bold(A)$ 一一对应
 ...
 - *正定二次型*：对任意非零向量，值恒为正的二次型，对应的实对称矩阵为*正定矩阵*（*半正定*：值恒非负）
  ...
  - 由于 $bold(x)^T (bold(A)^T bold(A)) bold(x)=(bold(A) bold(x))^T (bold(A) bold(x))>=0$，矩阵 $bold(A)^T bold(A)$ 半正定，若 $bold(A)$ 列满秩则正定
