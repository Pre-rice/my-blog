...
=== 内积空间
 - *内积*：二元运算，将线性空间中的两个向量映射到一个数域 $FF$（$FF=RR$ 或 $CC$）中的标量
  - 满足共轭对称性、第一变元线性、正定性
  - 通常讨论实内积空间，内积满足对称性、双线性、正定性
 - 内积赋予*几何度量*：长度（范数）、角度（进一步定义正交）、距离
 - *欧几里得空间* $RR^n$：$⟨bold(x),bold(y)⟩=bold(x)^T bold(y)$，正交变换保持内积，正交矩阵 $bold(Q)^T bold(Q) = bold(I)$
 ...
 - *平方可积函数空间* $L^2$：$⟨f,g⟩=integral f(x) overline(g(x)) d x$，是最典型的无穷维希尔伯特空间，也是傅里叶分析、量子力学和信号处理的基本框架。
=== 子空间
 - *运算*：和、交、补、直和、正交补
...
=== 相似对角化
 - *矩阵相似*： $bold(A)~bold(B)$ $<=>$ 存在可逆矩阵 $bold(P)$ 使得 $bold(B)=bold(P)^(-1) bold(A) bold(P)$
  - *理解*：相似矩阵表示同一线性变换在不同基下的表示， $bold(P)$ 为两组基的过渡矩阵
...
=== 二次型
 - *定义*：设 $bold(A)$ 是 $n$ 阶实对称矩阵，$bold(x)$ 是 $n$ 元列向量，则 $bold(x)^T bold(A) bold(x)$ 是一个二次型
  - $bold(x)^T bold(A) bold(x)=sum_(i=1)^n sum_(j=1)^n a_(i j) x_i x_j=sum_(i=1)^n a_(i i) x_i^2+2sum_(i<j) a_(i j) x_i x_j$
  - 二次型即 $n$ 个变量 $x_1, x_2, ..., x_n$ 的二次齐次多项式，二次型 $bold(x)^T bold(A) bold(x)$ 与其实对称矩阵 $bold(A)$ 一一对应
